...
function createMouse() {
   function generatMouse() {
    let posX = Math.round(Math.random() * (10 - 3) + 3);
    let posY = Math.round(Math.random() * (10 - 1) + 1);
    return[posX, posY];
   }

   let mouseCoordinates = generatMouse();
...
function move() {
    ...
    if(snakeBody[0].classList.contains('snakeBody')) {
        ...
        setTimeout(() => {
            alert(`Игра окончена. результат игры: ${score}`);
        }, 200)

        snakeBody[0].style.background = 'url(deadHead.jpg) center no-repeat'
...
/* Продолжи с того момента когда змее нужно начать двигаться в право мин. - 17:50 */
/* 22.06 Все готово!!! */
